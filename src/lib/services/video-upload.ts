    ...
    try {
      ...
      const videoData: Omit<FirestoreVideo, "id"> = {
        ...
        analysis: {
          ...
          aiMetadata: {
            detectedIngredients:
              result.analysis?.aiMetadata?.detectedIngredients || [],
            detectedTechniques:
              result.detectedContent.cookingTechniquesShown || [],
            confidenceScore: result.confidence,
            suggestedHashtags:
              result.analysis?.aiMetadata?.suggestedHashtags || [],
            equipmentNeeded: result.analysis?.aiMetadata?.equipmentNeeded || [],
            skillLevel: result.analysis?.aiMetadata?.skillLevel || "beginner",
            totalTime: result.analysis?.aiMetadata?.totalTime || 0,
            prepTime: result.analysis?.aiMetadata?.prepTime || 0,
            cookTime: result.analysis?.aiMetadata?.cookTime || 0,
            estimatedCost: result.analysis?.aiMetadata?.estimatedCost || {
              min: 0,
              max: 0,
              currency: "USD",
            },
